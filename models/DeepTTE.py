...
import utils
import models.Attr as Attr
import models.GeoConv as GeoConv
import models.SpatioTemporal as SpatioTemporal
...
def eval_on_batch_entire(pred, label, mean, std):
...
def eval_on_batch_local(pred, lens, label, mean, std):
...
class Net(nn.Module):
    def __init__(self, kernel_size=3, num_filter=32, pooling_method='attention', num_final_fcs=3, final_fc_size=128,
        ...
        # parameter of multitask learning component
    ...
    def forward(self, attr, traj, config):
        ...
        # sptm_s: hidden sequence (B * T * F); sptm_l: lens (list of int); sptm_t: merged tensor after attention/mean
        # pooling
