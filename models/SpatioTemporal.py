...
class Net(nn.Module):
    ...
    def mean_pooling(self, hiddens, lens):
        ...
        # lens = torch.FloatTensor(lens).to(device).float()
    ...
    def forward(self, traj, attr_t, config):
        ...
        # print(expand_attr_t.size())

        # concat the loc_conv and the attributes
        conv_locs = torch.cat((conv_locs, expand_attr_t), dim=2)

        lens = torch.FloatTensor(list(map(lambda x: x - self.kernel_size + 1, traj['lens'])))
        # print(type(lens))
