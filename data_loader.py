...
class MySet(Dataset):
    def __init__(self, input_file):
        content = open('./data/' + input_file, 'r').readlines()

        # 将content中的每一个元素装入json.loads(x)
        self.content = list(map(extract_json, content))
        # 同理
        self.lengths = list(map(extract_lngs_length, self.content))
...
def collate_fn(data):
    ...
    for key in stat_attrs:
        ...
        # 标准化 提前计算好各个变量的均值、方差。在config.json中。
    ...
    for key in traj_attrs:
        ...
        padded = np.asarray([np.pad(item[key], (0, max_length - len(item[key])))
                             for item in data])
