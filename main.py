...
from models.DeepTTE import Net
    ...
    for epoch in range(args.epochs):
        ...
        for input_file in train_set:
            ...
            for idx, (attr, traj) in enumerate(data_iter):
                ...
                print('\r Progress {:.2f}%, average loss {}'.format((idx + 1) * 100.0 / len(data_iter),
                                                                    running_loss / (idx + 1.0))),
                print()
            elogger.log('Training Epoch {}, File {}, Loss {}'.format(epoch, input_file, running_loss / (numOfiters + 1.0)))

        # evaluate the model after each epoch
        evaluate(model, elogger, eval_set, save_result=False)

        # save the weight file after each epoch
        weight_name = '{}_{}'.format(args.log_file, str(epoch)+'.pth')
...
def evaluate(model, elogger, files, save_result=False):
    ...
    for input_file in files:
        ...
        elogger.log('Evaluate File {}, Loss {}'.format(input_file, running_loss / (numOfiters + 1.0)))
