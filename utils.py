...
config = json.load(open('/Users/naoh/Desktop/DeepTTE/DeepTTE/config.json', 'r'))
...
def get_local_seq(full_seq, kernel_size, mean, std):
    ...
    # first_seq = torch.index_select(full_seq, dim=1, index=indices[:])
    second_seq = torch.index_select(full_seq, dim=1, index=indices[:-kernel_size + 1])
    # second_seq = torch.index_select(full_seq, dim=1, index=indices[:])
